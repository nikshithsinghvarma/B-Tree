...

{
	...
	public boolean Delete(KeyClass key, RID rid) throws DeleteFashionException,
		...
		//traversing tree for deletion.
		KeyDataEntry upEntry = _delete(key,rid,getHeaderPage().get_rootId());
		
		if(upEntry!=null)
		{
			BTIndexPage currentIndexPage = new BTIndexPage(getHeaderPage().get_rootId(),AttrType.attrInteger);

			KeyDataEntry tmpEntry1;
			RID delRID=new RID();
			
			if(((IndexData)upEntry.data).getData().pid==INVALID_PAGE)
			{
				for(tmpEntry1= currentIndexPage.getFirst( delRID);
						tmpEntry1!=null;
						tmpEntry1= currentIndexPage.getNext( delRID))
					{
					if(BT.keyCompare(tmpEntry1.key, upEntry.key)==0)
					{
						currentIndexPage.deleteSortedRecord(delRID);
					}
					}
			...
			else
			{
				...
				for(tmpEntry1= currentIndexPage.getFirst( delRID);
						...
						tmpEntry1= currentIndexPage.getNext( delRID))
					{
					if(((IndexData)tmpEntry1.data).getData().pid==((IndexData)upEntry.data).getData().pid)
					{
						currentIndexPage.deleteSortedRecord(delRID);
						currentIndexPage.insertKey(upEntry.key, ((IndexData)upEntry.data).getData());
						break;
	 ...
	 * it is not the id of the data entry)
	 *
	 *it is used to recurse through the tree for merging and redistribution
	 * @param key
	 *            the key in pair <key, rid>. Input Parameter.
	 * @param rid
	 *            the rid in pair <key, rid>. Input Parameter.
	 * @return KeyDataEntry if their is anything to be updated.
	 * @exception DeleteFashionException
	 *                neither full delete nor naive delete
	 * @exception LeafRedistributeException
	 *                redistribution error in leaf pages
	 ...
	 *
	 */
	private KeyDataEntry _delete(KeyClass key, RID rid,PageId currentPageId)
	...
	{		
		...
		//checking if while traversing page is leaf or index
		if(currentPage.getType() == NodeType.INDEX )
		{
			...
			//traversing until we reach leaf pages.
			upEntry=_delete(key,rid,nextPageId);
			
			if(upEntry!=null)
			{
				//checking if the sibling is deleted or not.(merged occured)
				if(((IndexData)upEntry.data).getData().pid==INVALID_PAGE)
				{
				...
				//checking if there is change in key.(redistribution occured)
				else
				{
		...
		{
			//on reaching the required leaf page.
			BTLeafPage currentLeafPage = new BTLeafPage(currentPage,AttrType.attrInteger);
			PageId currentLeafPageId = currentLeafPage.getCurPage();
			...
			for(tmpEntry1= currentLeafPage.getFirst( delRID);
					tmpEntry1!=null;
					tmpEntry1= currentLeafPage.getNext( delRID))
				{
				
					if(BT.keyCompare(tmpEntry1.key, key)==0)
					{
						try
						{
						currentLeafPage.delEntry(tmpEntry1);
						}
						catch(Exception e)
						{
			...
			//checking if redistribution or merge required.if current page has enough space
			if(currentLeafPage.available_space()<504)
			{
				return null;
			}
			else
			{	
				BTLeafPage siblingLeafPage;
				//if not last leaf page.
				if(currentPage.getNextPage().pid!=INVALID_PAGE)
				{
					siblingLeafPage = new BTLeafPage(currentPage.getNextPage(),AttrType.attrInteger);
	
					//if redistribution required.
					if(siblingLeafPage.available_space()<504)
					{
						
						KeyDataEntry tmpEntry;
						RID delNewRID=new RID();
						int i=currentLeafPage.available_space();
						//copy elements that are to be redistributed into current leaf
						for(tmpEntry= siblingLeafPage.getFirst( delNewRID);
								tmpEntry!=null;
								tmpEntry= siblingLeafPage.getFirst( delNewRID))
						{
							if( currentLeafPage.available_space()<=504 && siblingLeafPage.available_space()<=504)
							{
								break;
							}
							...
							siblingLeafPage.deleteSortedRecord(delNewRID);
							
						}
						//send changes to up entry
						upEntry=new KeyDataEntry(siblingLeafPage.getFirst(delNewRID).key, siblingLeafPage.getCurPage());
						return upEntry;
					...
					if(siblingLeafPage.available_space()==504)
					{
						...
						upEntry=new KeyDataEntry(siblingLeafPage.getFirst(delNewRID).key, new PageId(INVALID_PAGE) );
						//copying elements from sibling to current page.
						for(tmpEntry= siblingLeafPage.getFirst( delNewRID);
								tmpEntry!=null;
								tmpEntry= siblingLeafPage.getFirst( delNewRID))
						{						
						...
						//check for address 
						currentLeafPage.setNextPage(siblingLeafPage.getNextPage());
						//sibling page is not the end page
						if(siblingLeafPage.getNextPage().pid!=INVALID_PAGE)
						{
							BTLeafPage newSiblingPage=new BTLeafPage(siblingLeafPage.getNextPage(),AttrType.attrInteger);
							newSiblingPage.setPrevPage(currentLeafPageId);
						}
							siblingLeafPage.setNextPage(new PageId(INVALID_PAGE));
							siblingLeafPage.setPrevPage(new PageId(INVALID_PAGE));
						//	return the value to make changes at index.
						return upEntry;
